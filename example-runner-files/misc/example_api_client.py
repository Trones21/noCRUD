...
def exec():
    ...
    users = get_fixture(
        "users.json"
    )  # Ensure you set your fixtures path in fixtures.py
    creatingUser = users[0]["fields"]["username"]
    firstUser = APIClient()
    firstUser.login(creatingUser, "demo")

    # Create an actor
    filmObj = get_fixture_by_index(
        "films.json", 0
    )  # Pulls fields key directly from the object at param index
    ...
    if len(updatedfilm["title"]) == 0:
        failed_assertions.append(
            "Assertion failed: Banner message should have been populated on the update, but is empty"
        )
